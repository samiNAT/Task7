...
def load_tasks():
    ...
    if os.path.exists(TASKS_FILE):
        try:
            ...
            logging.error("Failed to load tasks. The file may be corrupted.")
            return []
    return []
...
def add_task(tasks, task_name):
    ...
    """
    if not task_name:
        logging.error("Task name cannot be empty.")
        return
...
def mark_task_done(tasks, task_index):
    ...
    if 0 <= task_index < len(tasks):
        if tasks[task_index]["done"]:
            logging.info(f"Task '{tasks[task_index]['name']}' is already marked as done.")
        else:
...
def handle_choice(choice, tasks):
    ...
    if choice == "1":
        ...
        if not tasks:
            logging.info("No tasks available to mark as done.")
            return True
        list_tasks(tasks)
        try:
